...
import edu.arizona.sirls.ontology_lookup.data.FormalRelation;
import edu.arizona.sirls.ontology_lookup.data.REntity;
...
public class SpatialModifiedEntityStrategy implements SearchStrategy {
	...
	public void handle() {
		...
		if(elocatorphrase.length()>0) {
			...
			ArrayList<EntityProposals> results = new EntitySearcherOriginal().searchEntity(elocatorphrase, "",originalentityphrase, prep);
			if(results!=null){
		...
		if(m.find()){
			...
			if(newentity.length()<=0){//e.g. search for 'ventral surface'
				...
				sentityps = new EntitySearcherOriginal().searchEntity(newentity, "", originalentityphrase, prep); //advanced search
			}
